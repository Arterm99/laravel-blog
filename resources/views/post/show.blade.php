...
@section('title')Страница Блога@endsection

@section('content')
    <main class="blog-post">
        <div class="container">
            <h1 class="edica-page-title" data-aos="fade-up">{{ $post->title }}</h1>
            <p class="edica-blog-post-meta" data-aos="fade-up" data-aos-delay="200">{{ $data->translatedFormat('F'),}} • {{ $data->day }} • {{ $data->year }} • {{ $data->format('H:i') }} • {{ $post->comments->count() }}Комментариев</p>
                ...
                <div class="col-lg-9 mx-auto">
                    <section class="py-3">
                        ...
                        <form action="{{ route('post.like.store', $post->id) }}" method="post">
                            ...
                            <!-- Урок 38 Для посчёта лайков-->
                            <span>{{ $post->liked_users_count }}</span>

                            <button type="submit" class="border-0 bg-transparent">

                                <!-- Если содержит пост, то сердци закрашенной -->
                    ...
                    <span class="username">
                        ...
                        <!-- diffForHumans() используется только с датами в Карбоне, показывает сколько минут назад был опубликован пост -->
                    ...
                    <!-- auth() - отображает блок тому, кто авторизовался -->
                    @auth()
